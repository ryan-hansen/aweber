...
class TestRequestLoggingMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_request_logging_middleware_success(self, mock_request, mock_response):
        ...
        with patch('src.app.middleware.log_request_info') as mock_log_request, \
    ...
    async def test_request_logging_middleware_exception(self, mock_request):
        ...
        with patch('src.app.middleware.log_request_info'), \
             patch('src.app.middleware.log_response_info') as mock_log_response:
...
class TestErrorTrackingMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_successful_request_no_tracking(self, middleware, mock_request):
        ...
        with patch.object(middleware, '_track_error_response') as mock_track_error:
    ...
    async def test_exception_tracking(self, middleware, mock_request):
        ...
        with patch.object(middleware, '_track_exception') as mock_track_exception:
            with pytest.raises(ValueError):
                await middleware.dispatch(mock_request, mock_call_next)
        
        mock_track_exception.assert_called_once_with(mock_request, test_exception)

    @pytest.mark.asyncio
    async def test_track_error_response_logging(self, middleware, mock_request):
...
class TestMiddlewareIntegration:
    ...
    def test_all_middleware_integration_validation_error(self, app_with_all_middleware):
        ...
        # Should get validation error from middleware
        assert response.status_code == 400
        data = response.json()
        assert data["error"] == "VALIDATION_ERROR"
        assert "Content-Type must be application/json" in data["message"]

    def test_all_middleware_integration_server_error(self, app_with_all_middleware):
        """Test all middleware work together for server error."""
        with TestClient(app_with_all_middleware) as client:
            with patch('src.app.middleware.log_request_info'), \
                 patch('src.app.middleware.log_response_info'), \
                 patch('src.app.middleware.logger'):  # Mock error tracking logger
