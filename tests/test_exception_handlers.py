...
class TestCreateErrorResponse:
    ...
    @patch('src.app.exception_handlers.settings')
    def test_error_response_production_mode_no_stack_trace(self, mock_settings):
...
class TestBaseAPIExceptionHandler:
    ...
    @pytest.mark.asyncio
    async def test_base_api_exception_handler_no_request_id(self, mock_request):
...
class TestValidationExceptionHandler:
    ...
    async def test_validation_exception_handler(self, mock_request):
        ...
        assert field_errors["number_of_parts"] == "ensure this value is greater than 0"

    @pytest.mark.asyncio
    @patch('src.app.exception_handlers.settings')
    async def test_validation_exception_handler_debug_mode(self, mock_settings, mock_request):
...
class TestHTTPExceptionHandler:
    ...
    def mock_request(self):
        ...
        request = Mock(spec=Request)
        request.url = Mock()
        request.url.__str__ = Mock(return_value="http://test.com/api/widgets/999")
        request.method = "GET"
        request.state = Mock()
        request.state.request_id = "test-request-123"
        return request
...
class TestGenericExceptionHandler:
    ...
    @pytest.mark.asyncio
    @patch('src.app.exception_handlers.settings')
    async def test_generic_exception_handler_production(self, mock_settings, mock_request):
    ...
    @pytest.mark.asyncio
    @patch('src.app.exception_handlers.settings')
    async def test_generic_exception_handler_debug(self, mock_settings, mock_request):
        ...
        content = json.loads(response.body)
        assert content["error"] == "INTERNAL_SERVER_ERROR"
        assert "Internal server error: Something went wrong" in content["message"]
...
class TestExceptionHandlerIntegration:
    ...
    def test_generic_exception_integration(self, app):
        ...
        pytest.skip("TestClient has middleware interaction issues with exception handlers")
